...
describe('SOKO print artifacts', () => {
  ...
  it('renders SOKO questionnaire forms with assignment and date data', () => {
    ...
    assert.doesNotMatch(html, /Zutreffendes ist[\s\S]*?angekreuzt/);
    assert.match(html, /left:18mm;top:258mm;width:16mm;height:16mm/);
    assert.match(html, /left:15mm;top:212mm;width:180mm;height:67mm/);
    assert.match(html, /left:15mm;top:166mm;width:180mm;height:44mm/);
    assert.match(html, />Bemerkungen<\/div>/);
    assert.match(html, /005 \/ 06/);
    assert.match(html, /left:104mm;top:22mm;width:91mm;height:42mm;[\s\S]*?005 \/ 06<\/div><\/div>/);
    assert.doesNotMatch(html, /Lfd\. Nr\. \/ Monat/);
    assert.match(html, /width:180mm;height:7mm;[\s\S]*?innerhalb von drei Wochen<\/strong> ausgef&uuml;llt/);
    assert.doesNotMatch(html, /Fragebogen<\/strong><br>/);
